...
from app.text_format import format_screen_body, p
...
async def build_home_panel_text(user_id: int, locale: str, *, variant: str = "menu") -> str:
    ...
    if not profile or not profile.sign:
        return format_screen_body(base)

    blocks = [
        format_screen_body(base),
        format_screen_body(
    ...
    if profile.relationship_status:
        ...
        blocks.append(format_screen_body(t(locale, "home_relationship_unset")))
    if profile.goal:
        blocks.append(format_screen_body(t(locale, "home_goal", goal=goal_display(locale, profile.goal))))
    else:
        blocks.append(format_screen_body(t(locale, "home_goal_unset")))
    if profile.mood_streak > 0:
        blocks.append(format_screen_body(t(locale, "home_streak", streak=str(profile.mood_streak))))
    blocks.append(format_screen_body(t(locale, "home_ref_promo")))
    return p(*blocks)
...
async def build_admin_stats_text(locale: str) -> str:
    ...
    sections.append(format_screen_body("\n".join(error_lines)))
