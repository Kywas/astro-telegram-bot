...
class Database:
        ...
        async with aiosqlite.connect(self._db_path) as db:
            ...
            async with db.execute(
                ...
                profile = UserProfile(
                    ...
                    compat_style=row["compat_style"] if row["compat_style"] else (row["horoscope_style"] if row["horoscope_style"] else (row["natal_style"] if row["natal_style"] else "plain")),
    ...
    async def set_compat_style(self, user_id: int, style: str) -> None:
        normalized = "plain" if style == "plain" else "terms"
        async with aiosqlite.connect(self._db_path) as db:
            await db.execute(
                "UPDATE users SET compat_style = ? WHERE user_id = ?",
                (normalized, user_id),
            )
            await db.commit()
