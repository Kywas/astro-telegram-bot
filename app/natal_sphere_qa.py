...
from app.natal_qa_synthesis import QaSynthFocus, finish_qa_body, format_qa_body
...
FINANCE_QUESTIONS: dict[str, tuple[str, str, str, str, str]] = {
    "ru": (
        ...
        "Куда вести карьеру и как реализоваться?",
        "Как складываются инвестиции и риск?",
        "Откуда приходит доход и финансовый рост?",
        "Где финансовые риски и типичные ошибки?",
    ),
    "en": (
...
KARMA_QUESTIONS: dict[str, tuple[str, str, str, str, str]] = {
    "ru": (
        "Какая кармическая тема звучит в моей карте?",
        "Что говорит карта о прошлых воплощениях?",
        "Какие уроки несёт Сатурн?",
        "Что тянет из прошлого Раху и Кету?",
        "Как прожить карму осознаннее в этой жизни?",
    ),
    "en": (
...
TRAITS_QUESTIONS: dict[str, tuple[str, str, str, str, str]] = {
    "ru": (
        "Кто я по сути — с чего начинается моя карта?",
        "Какой у меня темперамент и энергия?",
        "Как устроены мои эмоции и внутренний мир?",
        "Как я действую и общаюсь?",
        "Какие качества даны от рождения?",
    ),
    "en": (
...
LINEAGE_QUESTIONS: dict[str, tuple[str, str, str, str, str]] = {
    "ru": (
        "Что карта говорит о матери и отношениях с ней?",
        "Что карта говорит об отце и его влиянии?",
        "Какие темы рода и предков звучат в карте?",
        "Как семейные корни формируют мою опору?",
        "Где типичные сложности с родителями и родом?",
    ),
    "en": (
...
HEALTH_QUESTIONS: dict[str, tuple[str, str, str, str, str]] = {
    "ru": (
        "Как устроено моё тело и жизненная энергия?",
        "На что обратить внимание для здоровья?",
        "Как складываются силы и восстановление?",
        "Где типичные слабые места организма?",
        "Как заботиться о теле по карте?",
    ),
    "en": (
...
PURPOSE_QUESTIONS: dict[str, tuple[str, str, str, str, str]] = {
    "ru": (
        "В чём моё предназначение и главный путь?",
        "Какие таланты даны от рождения?",
        "Как реализовать себя и найти своё дело?",
        "Что помогает, а что мешает реализации?",
        "Куда вести энергию для максимального раскрытия?",
    ),
    "en": (
...
DHARMA_QUESTIONS: dict[str, tuple[str, str, str, str, str]] = {
    "ru": (
        "Какой мой духовный путь и дхарма?",
        "Как складывается связь с верой и смыслом?",
        "Что карта говорит о медитации и уединении?",
        "Какие духовные уроки несут Юпитер и Кету?",
        "Как жить в согласии с дхармой каждый день?",
    ),
    "en": (
...
TRAVEL_QUESTIONS: dict[str, tuple[str, str, str, str, str]] = {
    "ru": (
        "Есть ли в карте тема переезда или жизни за границей?",
        "Как складываются дальние путешествия?",
        "Куда тянет — какие направления и места?",
        "Что помогает, а что мешает переезду?",
        "Как чувствовать себя «дома» вдали от родины?",
    ),
    "en": (
...
UPAYA_QUESTIONS: dict[str, tuple[str, str, str, str, str]] = {
    "ru": (
        "Какие упайи подходят моей карте в целом?",
        "Как смягчить напряжённые планеты?",
        "Как усилить сильные стороны карты?",
        "Какие дни и практики поддержат баланс?",
        "Мантры, благотворительность и камни — что актуально?",
    ),
    "en": (
...
def finance_picker_intro(locale: str) -> str:
    ...
    if _lang(locale) == "ru":
        header = (
            ...
            "Деньги, рост и профессия в твоей карте — "
            "2-й дом (ресурсы), 10-й дом (карьера), 5-й и 11-й дома (риск и доход), "
            "Венера, Юпитер, Солнце и Сатурн.\n"
        )
    else:
        header = (
...
def karma_picker_intro(locale: str) -> str:
    ...
    if _lang(locale) == "ru":
        header = (
            ...
            "Кармические темы в твоей карте — "
            "12-й дом (прошлый опыт и отпускание), 8-й дом (глубинные уроки), "
            "9-й дом (дхарма), Сатурн, Раху и Кету.\n"
        )
    else:
        header = (
...
def traits_picker_intro(locale: str) -> str:
    ...
    if _lang(locale) == "ru":
        header = (
            ...
            "Врождённый темперамент и базовый «я» в карте — "
            "Лагна и 1-й дом, Солнце, Луна, Марс и Меркурий.\n"
        )
    else:
        header = (
...
def lineage_picker_intro(locale: str) -> str:
    ...
    if _lang(locale) == "ru":
        header = (
            "🌳 Род / Мать / Отец\n\n"
            "Род, родители и семейные корни в карте — "
            "4-й дом (мать, дом), 9-й дом (отец, род), Луна, Солнце и Юпитер.\n"
        )
    else:
        header = (
...
def health_picker_intro(locale: str) -> str:
    ...
    if _lang(locale) == "ru":
        header = (
            "💪 Здоровье / Тело\n\n"
            "Тело, энергия и восстановление в карте — "
            "1-й дом (тело), 6-й дом (здоровье и режим), 8-й дом (глубинные процессы), "
            "Солнце, Луна, Марс и Сатурн.\n"
        )
    else:
        header = (
            "💪 Health / Body\n\n"
...
def purpose_picker_intro(locale: str) -> str:
    ...
    if _lang(locale) == "ru":
        header = (
            ...
            "Смысл, дар и путь реализации в карте — "
            "9-й дом (дхарма), 5-й дом (талант), 10-й дом (дело), "
            "Солнце, Юпитер и Лагна.\n"
        )
    else:
        header = (
...
def dharma_picker_intro(locale: str) -> str:
    ...
    if _lang(locale) == "ru":
        header = (
            ...
            "Духовный путь и дхарма в карте — "
            "9-й дом (вера, смысл, наставничество), 12-й дом (мокша, уединение), "
            "Юпитер, Кету и Солнце.\n"
        )
    else:
        header = (
...
def travel_picker_intro(locale: str) -> str:
    ...
    if _lang(locale) == "ru":
        header = (
            ...
            "Переезд и поездки в карте — "
            "9-й дом (дальние путешествия), 12-й дом (жизнь за границей), "
            "3-й дом (дорога), Раху, Юпитер и Луна.\n"
        )
    else:
        header = (
            "✈️ Emigration / Travel\n\n"
...
def upaya_picker_intro(locale: str) -> str:
    ...
    if _lang(locale) == "ru":
        header = (
            "🪬 Гармонизация / Упайи\n\n"
            "Методы баланса по карте — дни планет, мантры, благотворительность "
            "и сознательная работа со слабыми и напряжёнными показателями.\n"
        )
    else:
        header = (
