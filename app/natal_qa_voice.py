...
def _plain_eli5_bridge(locale: str, first_sentence: str) -> str:
    ...
    if lang == "ru":
        ...
        if core_lower.startswith("ты строишь романтику"):
            return (
                "По-простому: тебе нужна связь, где тепло и честность, "
                "а не «мы вместе, потому что так удобнее всем вокруг»."
            )
        if "тебя тянет к типу" in core_lower:
            return (
                "Если на пальцах: сердце и привычка часто выбирают одно, "
                "а голова — другое. Карта показывает, к кому магнит сильнее."
            )
        if "брак и долгий союз" in core_lower:
            return (
                "Короче: речь не про дату «штампа», а про то, "
                "как ты держишь близость на длинной дистанции."
            )
        if "дом и «свои»" in core_lower:
            return (
                "По-человечески: дом — это где выдыхаешь, "
                "а не где играешь «идеальную семью для фото»."
            )
        if "в паре и семье не" in core_lower:
            return (
                "Суть простая: трения бывают у всех — "
                "важно заранее видеть свои «кнопки», а не ждать взрыва."
            )
        if "повторяющиеся «тяжёлые» сценарии" in core_lower:
            return (
                "Если на пальцах: жизнь не штрафует — "
                "она показывает один и тот же урок, пока ты не сменишь реакцию."
            )
        if "прошлое не обязано быть драмой" in core_lower:
            return (
                "Короче: это не обязательно «ты была Cleopatra». "
                "Скорее — внутренний шёпот, который слышен в тишине."
            )
        if "задержки и «не сейчас»" in core_lower:
            return (
                "По-простому: Сатурн — не злой дед с секундомером, "
                "а проверка «готов ли ты по-настоящему, а не на понтах»."
            )
        if "тяга к новому и привычка отпускать" in core_lower:
            return (
                "Если совсем просто: одна часть тянет вперёд, "
                "другая просит отпустить — и обе могут быть правы, если не путать с побегом."
            )
        if "осознанная жизнь — не только медитация" in core_lower:
            return (
                "Суть: осознанность — это выбор, который не стыдно "
                "объяснить близким за завтраком, а не только подушка для медитации."
            )
        if core_lower.startswith("с первого контакта ты читаешься"):
            return (
                "По-простому: первое впечатление о тебе — не маска для гостей, "
                "а твой «заводской режим», с которым ты входишь в жизнь."
            )
        if "стержень и напор — разные кнопки" in core_lower:
            return (
                "Если на пальцах: «кто я» и «давай уже» — "
                "две разные кнопки; жать обе сразу — рецепт усталости."
            )
        if "эмоции у тебя не «слабость»" in core_lower:
            return (
                "Короче: чувства — не баг, а навигатор. "
                "Иногда без интернета, но всё равно полезный."
            )
        if "как говоришь и как рискуешь" in core_lower:
            return (
                "По-человечески: слова и поступки — два канала связи; "
                "иногда один орёт, пока второй молчит — и это тоже ты."
            )
        if "сильные стороны — не медаль" in core_lower:
            return (
                "Суть простая: таланты — инструменты в ящике, "
                "а не медали на стене; слабые места — зоны, где нужен режим, не стыд."
            )
        if "мама и «дом» внутри" in core_lower:
            return (
                "По-простому: мама в карте — не только человек, "
                "а то, умеешь ли ты отдыхать внутри, даже когда вокруг шум."
            )
        if "отец и авторитет" in core_lower:
            return (
                "Если на пальцах: отцовская линия — про правила и опору; "
                "часть принимаешь, часть тихо переписываешь под себя."
            )
        if "род — не только фамилия" in core_lower:
            return (
                "Короче: род — это сценарии, которые передаются, "
                "как рецепт борща: у каждого своя вариация, но база узнаваема."
            )
        if core_lower.startswith("«свои» — это не обязательно кровь"):
            return (
                "По-человечески: «свои» — те, рядом с кем можно выдохнуть, "
                "а не обязательно те, кто в паспорте."
            )
        if "с родителями и родом не всегда" in core_lower:
            return (
                "Суть: с родными не всегда «как в рекламе» — "
                "и это не делает тебя плохим сыном/дочерью, просто живая история."
            )
        if "энергия — не бесконечный тариф" in core_lower:
            return (
                "По-простому: у каждого свой «бензобак» и свой стиль его опустошать — "
                "важно знать оба, а не только «я же могу ещё»."
            )
        if "режим — скучное слово" in core_lower:
            return (
                "Если на пальцах: сон, вода и рутина — не романтика, "
                "а то, что спасает, когда тело шлёт смс «я устал»."
            )
        if "восстановление — не лень" in core_lower:
            return (
                "Короче: отдых — не лень, а техобслуживание. "
                "Без него даже Ferrari начинает стучать."
            )
        if "слабые места — не приговор" in core_lower:
            return (
                "По-человечески: слабое место — не клеймо, "
                "а табличка «тут лучше не геройствовать, спасибо»."
            )
        if "тело любит баланс" in core_lower:
            return (
                "Суть простая: нагрузка + отдых — "
                "когда одно без другого, начинается «ну я же сильный» (спойлер: не всегда)."
            )
        if "предназначение — не табличка" in core_lower:
            return (
                "По-простому: «зачем я здесь» — не должность в LinkedIn, "
                "а то, к чему возвращаешься, даже когда устал."
            )
        if "таланты — не только «я рисую»" in core_lower:
            return (
                "Если на пальцах: талант — то, что у тебя выходит легче, "
                "чем у соседа, и без понтов «я гений»."
            )
        if "реализация — это когда" in core_lower:
            return (
                "Короче: своё дело — когда внешнее «что делаю» "
                "хотя бы иногда совпадает с внутренним «зачем»."
            )
        if core_lower.startswith("ты входишь в мир"):
            return (
                "По-человечески: твой «вход в жизнь» — тоже часть пути, "
                "не только резюме и KPI."
            )
        if core_lower.startswith("главная возможность сейчас"):
            return (
                "Суть: возможность не обязана быть громкой — "
                "иногда это тихий поворот, который стоит заметить."
            )
        if "духовный путь — не обязательно монастырь" in core_lower:
            return (
                "По-простому: духовность — не обязательно монастырь и чётки, "
                "иногда это честность в мелочах без зрителей."
            )
        if "смысл — когда действие" in core_lower:
            return (
                "Если на пальцах: смысл — когда поступок не предаёт твои ценности, "
                "даже если никто не похлопает."
            )
        if "медитация — не обязательно подушка" in core_lower:
            return (
                "Короче: тишина — не Instagram-поза, "
                "а пауза, когда мозг наконец выдыхает."
            )
        if "юпитер тянет расти" in core_lower:
            return (
                "По-человечески: рост и отпускание — не враги, "
                "просто разные дни в одной жизни."
            )
        if "дхарма в быту" in core_lower:
            return (
                "Суть: дхарма в быту — не только лотос, "
                "а «не вру себе в мелочах», когда никто не смотрит."
            )
        if "переезд — не «сбежать от проблем»" in core_lower:
            return (
                "По-простому: переезд — не магия «стану другим человеком», "
                "а смена декораций: с собой всё равно везёшь себя."
            )
        if "путешествия для тебя" in core_lower:
            return (
                "Если на пальцах: поездка для тебя — не только галочка на карте, "
                "а перезагрузка нервной системы (или её наоборот)."
            )
        if "чужие земли могут зажигать" in core_lower:
            return (
                "Короче: «далеко» для тебя — не одно и то же: "
                "где-то оживаешь, где-то устаёшь быстрее, чем успел сфоткать."
            )
        if "переезд тормозят не только визы" in core_lower:
            return (
                "По-человечески: мешает не только бюрократия — "
                "страх и привычка тоже умеют держать на месте."
            )
        if core_lower.startswith("«дом» вдали от родины"):
            return (
                "Суть: дом вдали — не GPS-точка, "
                "а ощущение, которое можно собрать из мелочей, а не из штукатурки."
            )
        if "гармонизация — не «купи камень" in core_lower:
            return (
                "По-простому: упайи — не магический шопинг, "
                "а режим, честность и простые действия, которые реально работают."
            )
        if "смягчить напряжение — не «стать другим" in core_lower:
            return (
                "Если на пальцах: цель — не новая личность, "
                "а чуть тише привычная реакция, которая всех бесит."
            )
        if "усилить сильное — не чинить" in core_lower:
            return (
                "Короче: не чини то, что держит — "
                "лучше подпирай то, что у тебя и так работает."
            )
        if "дни планет — как расписание" in core_lower:
            return (
                "По-человечески: дни планет — скучное расписание, "
                "но мозгу проще, когда есть «день доброты» и «день терпения»."
            )
        if "мантры, дана и камни — не шопинг" in core_lower:
            return (
                "Суть: мантра и доброе дело — в начале списка, "
                "камень — в конце и только с тем, кто в теме."
            )
        if "с деньгами у тебя не «просто счёт" in core_lower:
            return (
                "По-простому: деньги — не только цифры в приложении, "
                "а то, что ты реально ценишь и куда вкладываешь душу."
            )
        if "карьера — не только должность" in core_lower:
            return (
                "Если на пальцах: работа — не только строка в резюме, "
                "а «зачем встаю утром» (иногда ответ приходит только после кофе)."
            )
        if "риск и «быстрые деньги»" in core_lower:
            return (
                "Короче: «быстрые деньги» звучат красиво в рекламе — "
                "у тебя своя скорость, и форсировать не обязательно."
            )
        if "доход растёт не от молитвы" in core_lower:
            return (
                "По-человечески: рост дохода — не лотерея, "
                "а связка навыка, дисциплины и «не сливать на импульс»."
            )
        if "финансовые риски — не «ты неудачник»" in core_lower:
            return (
                "Суть: риск — не приговор, а привычка: "
                "импульс, страх или вера в «лёгкие деньги» — карта подсвечивает, где."
            )
        return (
...
def _build_summary(locale: str, sentences: list[str], markers: tuple[str, ...]) -> str:
    lang = _lang(locale)
    if len(sentences) >= 2:
        detail = " ".join(sentences[1:]).lower()
        last = sentences[-1].strip()
        if last and last.lower() not in detail and len(last) < 220:
            return last
        if lang == "ru":
            return "Суть та же: смотри, где это уже проявляется в твоей жизни — там и ответ."
        return "Same idea: notice where this already shows up in your life — that's the answer."
    if markers:
...
def compose_plain_qa_answer(
    ...
    if question.strip():
        ...
        if echo and intent not in {"how_relationship", "who_partner", "challenge", "health", "purpose", "strength", "money", "career"} and echo.lower()[:50] not in core.lower():
...
def plain_topic_hook(locale: str, question: str, *, hint: str = "") -> str:
    ...
    if lang == "ru":
        ...
        elif any(w in q for w in ("сильн", "талант", "ресурс", "опор")):
            base = "Начну с главного — про твои сильные стороны (не медаль, а инструменты на каждый день)"
